   ...
   extends DetailPanel
 {
   ...
   protected void initDefinitionsTable() {
     ...
     {
       this.runningDefinitions = new HashMap();
       for (HistoricProcessInstance instance : this.instanceList) {
         ...
         if (this.runningDefinitions.containsKey(processDefinitionId)) {
           managementDefinition = (ManagementProcessDefinition)this.runningDefinitions.get(processDefinitionId);
         }
         else {
       ...
       this.definitionsTable.addListener(new Property.ValueChangeListener() {
         ...
         public void valueChange(Property.ValueChangeEvent event) { Item item = AdminRunningInstancesPanel.this.definitionsTable.getItem(event.getProperty().getValue());
           if (item != null) {
             String definitionId = (String)item.getItemProperty("id").getValue();
             AdminRunningInstancesPanel.this.selectedManagementDefinition = ((AdminRunningInstancesPanel.ManagementProcessDefinition)AdminRunningInstancesPanel.this.runningDefinitions.get(definitionId));
   ...
   protected void initInstancesTable()
   {
     ...
     this.instancesTable.addListener(new Property.ValueChangeListener() {
       ...
       public void valueChange(Property.ValueChangeEvent event) { Item item = AdminRunningInstancesPanel.this.instancesTable.getItem(event.getProperty().getValue());
         if (item != null) {
   ...
   protected void addProcessImage(ProcessDefinition processDefinition, HistoricProcessInstance processInstance)
   {
     ...
     if ((processDefinitionEntity != null) && (processDefinitionEntity.isGraphicalNotationDefined()))
     {
       ...
       StreamResource diagram = new ProcessDefinitionImageStreamResourceBuilder().buildStreamResource(processInstance.getId(), processInstance.getProcessDefinitionId(), this.repositoryService, this.runtimeService, diagramGenerator, processEngineConfig);
   ...
   protected void addTasks(HistoricProcessInstance processInstance)
   {
     ...
     List<HistoricTaskInstance> tasks = ((HistoricTaskInstanceQuery)((HistoricTaskInstanceQuery)((HistoricTaskInstanceQuery)((HistoricTaskInstanceQuery)this.historyService.createHistoricTaskInstanceQuery().processInstanceId(processInstance.getId())).orderByHistoricTaskInstanceEndTime().desc()).orderByTaskCreateTime()).desc()).list();
   ...
   protected void addVariables(HistoricProcessInstance processInstance)
   {
     ...
     Map<String, Object> variables = new TreeMap(this.runtimeService.getVariables(processInstance.getId()));
     ...
     if (!variables.isEmpty())
     {
       ...
       for (String variable : variables.keySet()) {
         Item variableItem = this.variablesTable.addItem(variable);
